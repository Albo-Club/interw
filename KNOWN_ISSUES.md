...
the next in English, and the provider detects the language per answer. Mistral
also documents `language` as incompatible with `timestamp_granularities`, and
the segment timestamps are what anchor every quote in a report.
