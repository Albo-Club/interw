...
above 282 KiB gzip for what `/s/$token/interview` loads: the preloads TanStack
...
- **Raised from 280 to 282 KiB on 2026-09-25**, at 280.3 KiB, when the
  interview stage (#71) met two changelog entries on `main`. The recruiter
  changelog is the lever: `src/lib/i18n.ts` imports every namespace
  statically, so the whole `changelog` namespace (~13.6 KiB gzip, en + fr)
  ships to every candidate, and **each new changelog entry grows the
  candidate page**. Loading `changelog` in the recruiter app only is the
  reduction to do next.
