...
| C6  | Spam 30 messages in 1 min                               | `chatSend` rate-limit triggers (also gates approvals). A single message over 8 000 characters is refused (`prompt_too_long`) before it is stored or counted. `POST https://<deployment>.convex.site/api/chat` → `404`: no chat entry point outside the metered mutations |
