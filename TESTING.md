...
| IA6 | Publish gate (M10) | Try to publish (wizard **and** role page) with: no question; a question left at the seeded example text; a criterion left at its example name ("Technical depth"); no criterion | Each refused, by `projects.publish` too, not just the disabled button. The review step lists every blocker with its number ("Question 2 still has the example text"). The example text is recognised in both languages. `convex/projects.test.ts` |
