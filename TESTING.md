...
| B9 | Access audit | `pnpm audit:access:check` | Exit 0. Fails on any **public** Convex function with no access check before its first write — followed through helpers and delegated `internal.*` functions — and on any public function listed in `convex/_generated/api.d.ts` that the script cannot read. Run `pnpm audit:access` to print the full matrix; deliberate exceptions are declared with a `// access: <reason>` comment above the export and are listed in the output |
...
| A1  | `/register` (→ `/login?mode=signup`, "Create your account") → email → code → "What should we call you?" → onboarding org "Acme" | Redirects to `/app/acme`, user created. `superAdmin: true` only if the address equals `SUPER_ADMIN_EMAIL` on the Convex deployment — whoever signs up first gets nothing without it. If `DEV_NOTIFY_EMAIL` is set, a "[interw] New signup: …" email arrives in that inbox (1× per new user, not on re-login). |
...
| A10 | Rate-limit                                              | Password sign-in 6× /60s, or 4 code requests /60s from one browser → "Too many attempts…". 6 password sign-ins to **one address** in a row, from any mix of browsers or networks → "Too many attempts…" on the 6th even with the right password, and the same for an address with no account; an email code still signs in. 4 codes to **one address** within the hour → "We've already sent several emails to this address…" — same answer for an address with no account. Same on "Forgot your password?" and "Resend verification email" |
...
| A12a | Verification link: password only | Open a legacy sign-up verification link signed out | The form shows the password field, "Forgot your password?" and nothing else: no "Email me a code instead", no Google button — either would delete the password just chosen |
...
| SH5b | **Dashboard past 400 sessions** (Back M3) | With more than 400 invitations in 30 days, **Invited** and **Completed** read `400+`, never a smaller total. A decision on an older completed interview still counts under **Decisions so far**; the latest completed interview heads **Recently completed**. `convex/dataCorrectness.test.ts` |
...
| SH10 | AI button in header (or ⌘J / Ctrl+J)                          | Toggles the AI panel (desktop column from `lg` / modal sheet below); state persists via the `ai_panel_state` cookie |
...
| SH12 | Unknown URL (e.g. `/app/acme/nope` or `/nope`)                | `/nope`: styled 404 card (FR/EN by locale) + back-home button. `/app/acme/nope`, `/app/acme/projects/typo`, `/app/acme/candidates/<bad id>`: **Nothing here** inside the app layout (sidebar kept), with **Back to roles**, and nothing sent to Sentry (M1) |
| SH12b | Translated errors (M3) | As a plain member, trigger a refused action (e.g. `projects:setTeam` from the console on a role you did not create) | A specific, translated message ("Only admins, owners and the role's creator…"), never "Something went wrong". `src/lib/convex-errors.test.ts` fails if any code thrown in `convex/` lacks an en or fr message |
| SH13 | Cold load of `/app` and `/app/{org}`, then Dashboard / Roles | While the session and org resolve, a skeleton of the shell (sidebar, header, KPI row, table) — no "Loading…" line (M11). Then animated skeletons for KPI, recent candidates and table. Super admin lists show skeleton rows |
| SH16 | Reduced motion and focus (chantier 4 #7) | Turn on *Reduce motion* in the OS, open a dialog, a menu, the sidebar; then Tab into an assistant reply | Nothing slides or fades, spinners still turn (M7). The assistant's **Copy** button appears when focus reaches it, and is always visible on a touch screen (F5). KPI figures line up digit for digit. Text renders in Inter from our own origin (DevTools → Network: `inter-latin-wght-normal-*.woff2`, no third-party font request, M6). `src/styles/design-pass.test.ts` |
...
| I4b | A member opens a **colleague's** pending invite link        | Lands on `/app/<org>` as for their own link, and the invitation is still pending in Settings → Team: the colleague's link still joins them |
...
| I10 | Email-match casing — invite `Bob@Test.local`, accept signed in as `bob@test.local` | Accepted (ASCII case and surrounding spaces are ignored on both sides). Non-ASCII letters are compared as typed: see `KNOWN_ISSUES.md` § "Addresses fold ASCII case only" |
| I11 | Sign up from `/register?redirect=/accept-invite/<token>` (not the inline accept page) | The address field shows the invited address, read-only, and the code step offers "Back" instead of "Use a different email". Lands **in the org** (`/app/<org>`), not stuck on `/app`: signup → signin → full nav to the accept page, which attaches the member. Parity with the inline `/accept-invite` flow |
| I12 | **Inviter loses the right to invite** — admin Bob invites `carol@test.local`; then remove Bob (or make him a member) | Carol's link shows the "invitation not found" card, and accepting refuses; she no longer sees it on onboarding. Settings → Invitations: the row reads "No longer valid" with the hint to invite again, no Resend and no Copy link, only Revoke. Inviting `carol@test.local` again works and replaces the row. `convex/invitations.test.ts` |
...
| L14 | The only super admin → Security tab                    | "You're the last super admin" notice, delete button disabled. A direct `POST /api/auth/delete-user` answers 400 `LAST_SUPER_ADMIN` and mails nothing; a link requested earlier lands on "Your account wasn't deleted". Make another user super admin (SA3) → notice gone, deletion goes through. `convex/accountLifecycle.test.ts`, `convex/users.test.ts` |
...
| SA2 | List all users and organizations across all tenants | 25 per page with **Load more** (Back F8: paginated, no whole-table read). The four counters read `1000+` past their scan bound |
...
| #   | Step                                                    | Expected result                                                   |
| --- | ------------------------------------------------------- | ----------------------------------------------------------------- |
| C1  | Open `/app/acme` in a fresh browser (no `ai_panel_state` cookie) | AI panel **closed**; DevTools › Network shows no `chunk-…` holding streamdown until the panel opens. Open it: rounded column on the right (≥ `lg`); latest thread resumed, else empty state with suggestions; the composer takes focus |
| C1a | Same, window narrower than 1024 px (or a phone)           | The panel opens as a sheet over a dimmed page: Tab cycles inside it only, Escape or a click on the backdrop closes it and focus returns to the AI button; a screen reader announces a dialog named "AI assistant" |
...
| C2c | Ask it to reply with `![chart](https://example.com/x.png)`, then with `<img src="https://example.com/x.png" alt="chart">` | Both show "[Image not shown: chart]" ("[Image non affichée : chart]" in FR); DevTools › Network shows **no** request to example.com. A link to the app (`/app/acme/projects`) is clickable and opens in the same tab; a link elsewhere (`https://example.com`, `mailto:`) shows as plain text with no href |
...
| C6  | Spam 30 messages in 1 min                               | `chatSend` rate-limit triggers (also gates approvals). A single message over 8 000 characters is refused (`prompt_too_long`) before it is stored or counted. In the composer, a counter `n / 8,000` appears as soon as you type; past the limit it turns red with "Too long to send." and Send is disabled (FR: « Trop long pour être envoyé. »). `POST https://<deployment>.convex.site/api/chat` → `404`: no chat entry point outside the metered mutations |
...
| S3 | Security headers present (CSP, HSTS, etc.)         | `curl -I http://localhost:3000` → expected headers. Specifically `permissions-policy: camera=(self), microphone=(self)` — an empty `camera=()` denies the app's **own** camera — and a CSP `media-src` listing the bucket origin and `blob:` (B4 asserts both). `img-src` is `'self' data:`, the Convex deployment origin, the bucket and `https://*.googleusercontent.com` — **no bare `https:`**. Then check the images still load: your avatar and the org logo (sidebar, org switcher, members page, the candidate page `/s/<token>`), a Google sign-in avatar if Google is enabled, and question videos play. A malformed `MEDIA_ORIGIN` (`…scw.cloud; script-src *`) is dropped, not spliced: `media-src` falls back to `https:` and no extra directive appears |
...
| S11b | **Typed names stay text** | Set your display name to `Eve</strong><strong>Admin` (Account), then invite someone and open their link; rename the org to `Acme` + a line break + `Bcc: x@test.local` via the API (`organizations:updateGeneral`) | The accept page shows the name exactly as typed, one bold run, no extra bold or line break; `Tom & Jerry` shows as `Tom & Jerry`, never `&amp;`. The org name is stored on one line, and every email subject stays on one line. Force a failed `/api/auth` call: the log line is `path=`, never a query string. `src/lib/i18n.test.ts`, `convex/emailTemplates.test.ts` |
| S12 | **Auth client IP comes from the platform** | Sign in on the deployed app, then `/app/me` → Sessions | The session shows your real public IP. In the Convex logs, no `Rate limiting could not determine a client IP` warning for requests through the web domain. See `KNOWN_ISSUES.md` § "Brute force: the IP is a claim, the account is not" for what this does not cover |
...
| -- | -------- | ----- | -------- |
| IA1 | Create a role | `/app/{org}/projects` → New role → job title → Create. Again with **Add an internal name** | One field, no language and no team. Lands on the wizard's **Questions** step (`?step=` in the URL survives a reload), status **Draft**. Without an internal name the list shows the job title; with one, the list shows it and the candidate never does |
...
| IA2b | **Interview length follows the questions** | Set three answer times to 1 min, 2 min and 5 min | The banner reads **Maximum interview length 10 min** and updates as each is changed; the Publish step, the role page, the invitation ("Allow up to 10 minutes") and the candidate's welcome screen all say 10. There is no length field to type anywhere. `convex/lib/interviewDuration.test.ts`, `convex/candidateEmail.test.ts` |
...
| IA6 | Publish gate (M10) | Try to publish (wizard **and** role page) with: no question; a question left at the seeded example text; a criterion left at its example name ("Technical depth"); no criterion | Each refused, by `projects.publish` too, not just the disabled button. The Publish step lists every blocker with its number ("Question 2 still has the example text"), each one a link to the step that fixes it; the stepper marks Questions and Evaluation done or to complete. The example text is recognised in both languages. `convex/projects.test.ts` |
| IA6b | Delete a role (E6) | As the creator (or an admin), on a role nobody was invited to: **Delete** → confirm | Lands on the roles list with "Role deleted"; its questions, criteria, team rows and recordings are gone. On a role with candidates, the dialog explains it can only be archived and offers no delete. A plain teammate sees no Delete button, and the server refuses them (`insufficient_role`) |
| IA7 | Import a job ad | Questions → Import from a job ad → paste a real published ad URL | Draft appears with the requested number of questions and criteria summing to 100. **Nothing is saved** until "Add all to the role", which writes the whole draft in one mutation (`jobImport.applyDraft`): if the role would exceed 25 questions or 12 criteria, nothing is added and the error says why (F9). `convex/projects.test.ts` |
...
| IA9 | Role team | Create a role, then wizard → Publish → **Choose the team…** → tick one colleague A; open it as A and as another plain member B | A sees it; B does not see it in the list, in search, or by URL — and gets **not found**, not "forbidden". An admin who is not on the team still sees it. Your own row is ticked and locked ("Creator of the role"). The role's URL ends in six random characters, even when another role (visible or not) has the same title — never `-2` (T17-3). In the roles list, an owner or admin sees **On the team** next to the roles whose team they are on — the ones that email them (F6) |
...
| IA9d | **Leaving a team takes its links and threads** | Put member A and an admin on a role's team. As each, create a share link on one of its reports and ask the assistant about one of its candidates. Untick both, save | A's share link now shows "This link was revoked" and A's assistant conversation about that candidate is gone; A's links and conversations about other roles stay. The admin, who still sees every role, keeps both. `convex/teamAccess.test.ts` |
| IA9e | Creator seats of existing roles | After deploying, Convex dashboard → Data → `migrations` | Within an hour the `backfillCreatorSeats` row has `doneAt`, and every role whose creator is still a member has a `projectShares` row for them. Until then a plain member does not see the roles they created before the deploy — run `migrations:backfillCreatorSeats` from the dashboard right after the deploy to close that window |
...
| #  | Scenario | Steps | Expected |
| -- | -------- | ----- | -------- |
| IB1 | Invitation | Role → Candidates → Invite → one name + address, once on a role with a job title shown to candidates and once on a role without one | Email arrives; the link is `/s/{token}`. The subject and body name the role by its job title shown to candidates; without one they read **Your interview with {organisation}** — never the role's internal title. `convex/candidateEmail.test.ts` |
| IB1b | **Public candidate link** | Active role → **Create candidate link** → **Copy candidate link**; open it in a private window, enter a name + address, **Start** | Lands on the `/s/{token}` welcome screen greeting that name; no email is sent. The candidate shows up in the role's table like an invited one and goes on through IB4 → IB16 unchanged. Submitting the same address again opens a **second** session, never the first one back; inviting that address from the recruiter side also creates a new session, and the self-applied row offers no **Resend**. Button hidden on a draft, archived or past-deadline role. `convex/apply.test.ts` |
| IB1c | Public link closed | Archive the role (then, separately, set its deadline to yesterday) and reopen `/apply/{token}`; also open `/apply/short` | "This interview isn't open" / "This interview has closed", with no form; the short token gives "This link doesn't work". `noindex, nofollow` in the page head |
...
| IB4 | Welcome screen | Open the link | On a first visit to a role with a recorded intro, the recruiter's video opens the page, before consent — it plays by itself or behind **Play the question** where the browser refuses; a candidate coming back to finish does not get it again. Greeting, role, question count, "Up to N minutes" (IA2b), and a **What you'll need** block (camera and microphone, a quiet place, keep the page open). The only secondary link is **Your data**, in the footer. No app navigation anywhere on the page |
| IB4b | Role language | Create a role with the app in French, open its link in a browser set to English. Then press **English** in the header | The whole candidate surface is in French — welcome, check, interview, thank-you page and data page. It follows the role (the recruiter's language at creation), not the browser. The header button switches every screen to English for the rest of the tab, without touching the `lang` cookie |
| IB4c | Intro on the welcome screen only | Invite a candidate on a role set to **No introduction**, then on one set to **Video of you** with nothing recorded, then on one with a recorded intro | The first two show no video on the welcome screen. The third shows it above the greeting. In every case the device check goes straight to question 1 — the interview has no intro screen any more |
...
| IB6b | **The CV key stays on the server** | In DevTools → Network, upload a CV and inspect the `candidate:requestDocumentUpload` response and the `candidate:attachDocument` request | The response carries `uploadUrl` and `contentType` only; attach sends `mimeType`, never a key. Replacing a PDF with a DOCX leaves `cvKey` on `…/cv.docx` and the PDF deleted. `convex/documentGate.test.ts` |
| IB6c | **Unresolved tokens never reach the limiter** | Call `interview:promptMediaUrls` 32 times with a random 43-character token | Every call answers `not_found`, never `rate_limited`, and no rate-limiter row is written: `candidateWrite` is keyed on the resolved session. `convex/interview.test.ts` |
| IB7 | Device check | Open the check screen on a phone and a laptop, then deny camera permission | The screen is one video stage like the interview's: the camera fills it, the mic readout sits on it, and it fits without scrolling. Denied, the stage explains how to allow it in the address bar, the whole sentence, never cut — never a blank screen |
...
| IB7c | Chosen devices are used | On the check screen open **Settings** under the stage, pick a second microphone, then continue | The interview URL carries `?mic=…`, and the recording is from the microphone picked — not the system default |
...
| IB7e | **Answer size and slot lifetime follow the question** | Set a question to 60 s, record it, and read the `requestSegmentUpload` response URLs | `X-Amz-Expires=240` (answer length + 3 min), not 900. A declared video over ~31 MB for that question is refused `media_too_large` before any upload. `convex/interview.test.ts` |
...
| IB8b | **Plain HTTP** (Cand F3) | Open the candidate link over `http://` on a LAN IP (e.g. `http://192.168.x.x:3000/s/{token}/check`) | The check screen says the page wasn't opened over a secure connection and to use the link exactly as emailed (`https://`) — never "this browser can't record". `src/lib/media/devices.test.ts` |
| IB9 | Mic meter | Speak, then stay silent | The readout in the stage's top-left corner moves and reads "picking you up"; silence reads "can't hear anything" — and the **Start anyway** button is still available |
...
| IB9c | Practice take | Record a 10-second test, then play it back | The take fills the stage with your live camera in its corner, still without scrolling; you see and hear yourself; nothing appears in the bucket or in `segments`. "Record another test" replaces it |
| IB10 | Record an answer | Start my answer → speak → I've finished my answer | While the question is asked, the candidate's camera is a thumbnail in the stage's corner; once recording starts it fills the stage, with the question as a caption. The whole screen fits without scrolling on a phone and a laptop. A recorded question plays by itself — on iPhone Safari it may stay paused behind a **Play the question** button, which plays it. The preview shows the candidate **throughout** the recording, never a black box — portrait on a phone held upright. Saving shows a percentage. Both an audio and a video object appear under `orgs/{orgId}/sessions/{sessionId}/q0.*` — the video is `q0.mp4` on Chrome, Edge and Safari, `q0.webm` on Firefox |
| IB11 | Time limit | Set a question to 30 s, then say nothing and wait | The time left (`m:ss`) and a bar along the stage's bottom edge show from the start; both turn to a warning at 30 s remaining; recording stops on its own; the answer is saved |
| IB12 | **Network cut mid-answer** | Start an answer, disable the network, finish the answer | Shows "your last answer didn't save" over the stage with **Try again** and **Skip**, and the start button steps aside. Re-enable the network → Try again → it uploads |
...
| IB12h | **Back or close with an unsent answer** (Cand M12) | During a recording, while saving, and on "your last answer didn't save": press the browser's Back button, then try to close the tab (Safari too) | Back asks "Your answer hasn't been sent yet… Leave anyway?" — Cancel keeps the page and the answer; closing the tab gets the browser's leave prompt in all three states. Leaving anyway writes a `recording_abandoned` row to `sessionEvents` (detail `recording` or `unsent`). Outside those states nothing asks. `src/lib/interview-machine.test.ts`, `convex/interview.test.ts` |
| IB12i | **Each screen names itself** (Cand M10) | Walk welcome → check → interview → done, then open the privacy page | Each tab title differs ("Welcome — your interview", "Camera and microphone check — …", "Questions — …", "Thank you — …", "Your data — …", French under a French locale), each page has a `<meta name="description">`, and `noindex, nofollow` is still there. `src/components/candidate/screenHead.test.ts` |
...
| IB14b | **Sessions expire with their role** (B6) | Role with a pending and an in-progress candidate, expiry passed more than 24 h ago; wait for the hourly cron (`sessions:expireOverdueSessions`) | Both sessions read **Expired** in the candidate table and the status filter finds them; the ⋯ menu offers no resend, copy link or cancel; the candidate page says **Interview window closed**; the link keeps saying the interview has closed even after the deadline is pushed back. Within the first 24 h nothing changes, so a candidate can still finish. `convex/dataCorrectness.test.ts` |
| IB14c | **No invitation past the deadline** (PR #50) | Role whose deadline passed (inside the 24 h grace), with a pending candidate | No **Invite** button in the header nor in the empty table, and the ⋯ menu offers no **Resend**. Calling `sessions:invite` or `sessions:resendInvitation` directly is refused `project_expired`. Push the deadline back → both come back. On an archived role, **Resend** is absent too and the server refuses it `project_archived`. `convex/dataCorrectness.test.ts` |
...
| IC3 | **Evidence anchoring** | Open a fresh report (new tab, cache disabled) and immediately click a quote's timestamp late in a long answer, e.g. 3:30 | The player switches to the right answer, shows "Loading the answer… n%" while the answer downloads, then starts at the quoted second — not at 0:00, not a few seconds off. Do it on Chrome **and** Safari (macOS and iPhone), on answers recorded in Chrome, Safari **and** Firefox. On an iPhone, if playback does not start by itself, a "Play from 3:30" button must appear — never a click that does nothing |
| IC3c | **Answer length and a failed download** | Open a report and watch the network tab for 50+ minutes; then block the bucket host in DevTools and switch answer | Each answer's chip shows its length (`Question 2 · 4:12`) from the first render, whatever the browser's own controls say — on `/r/…` only once the server has measured it, never the candidate's own figure. The 50-minute re-signing downloads **nothing** again. With the bucket blocked the answer shows "This answer could not be loaded", not an endless spinner, and downloads again once the page has a fresh URL |
...
| IC11b | **Relaunch from the candidate page** | On a completed session with a failed step and no report, open the candidate page as the role's creator → **Relaunch the analysis**. Then as a team member who is not creator/admin. Then click it four times in a row as the creator | A `relaunch` row with the creator's `actorId` appears in `jobLog` and the pipeline trail shows **Relaunch**. The team member sees no button (the server answers `insufficient_role`). The fourth click says to wait (`reportRelaunch` bucket). Relaunching while transcriptions are still running does not queue a second job for an answer already in flight: one `started` or `skipped` transcribe row per answer |
...
| IC8b | **A role left with nobody is not silent** (PR #43) | Member B creates a role alone (no team), then is removed; complete an interview on it | The org's admins and owners get "Report ready". On a role with at least one team member still in the org, only the team is mailed, as in IC8. `convex/guards.test.ts` |
...
| ID1h | Document download names | FR then EN interface: download a candidate's CV and cover letter | Saved as `CV - <name>.pdf` and `Lettre de motivation - <name>.pdf` / `Cover letter - <name>.pdf`, with the stored extension. `convex/candidatePage.test.ts` |
| ID1d | Decision history | As two different members, set Shortlisted, then Rejected, then clear it. Open **History** under the decision | Three rows, newest first, each with who and when (a removed account reads **A former member**). Clicking the current decision again adds no row. After erasure (IE2) no `decisionEvents` row names the session |
| ID1e | **Playback survives a note and an hour** | Play an answer to 1:30, type a note and leave the field. Then leave the tab open more than 50 minutes (or block the bucket host in DevTools and press play) | The video keeps playing at 1:30 after the note is saved. After 50 minutes the URLs are re-signed without the position resetting; if the bucket fails, **The recordings could not be loaded** shows with **Load them again**, on the candidate page and on `/r/…` alike |
| ID1f | Highlights, documents, empty strengths | Open a report with highlights, a CV and a cover letter; then one whose `strengths` is `[]` | **Worth watching** lists the moments under the player, each starts playback at its second. **CV** and **Cover letter** open in a new tab. The Strengths card shows "—" on the candidate page and on the share link. The AI disclaimer is still there |
| ID1g | Delete is admin-tier | Open a candidate page as a team member who is not creator/admin, then as the creator | No **Delete this candidate's data** button for the team member; the creator has it |
| ID2 | Private note | Type a note, blur | Saved. Never appears on any candidate or shared surface |
| ID1b0 | **Resend refused on a bounce** (Pipe F9) | ⋯ → Resend the invitation on a row that shows **Invitation bounced** (or spam) | Refused with "This address bounced or marked our email as spam…"; no email leaves. A late `email.sent` webhook after a bounce leaves the row **bounced**. `convex/dataCorrectness.test.ts` |
...
| ID1b1 | **Bounces flagged per candidate** (PR #45) | Invite `bounced@resend.dev` on a role, then send 200+ other emails in the org (a bulk invitation on another role) | The bounced row still says **Invitation bounced**; re-sending to a corrected address that is delivered clears it. `convex/sessions.test.ts` |
...
| ID3b | **Audio answers play as audio on a share link** | Record one answer with the camera covered or unplugged (audio only), then open the report's share link | That answer plays in an audio player, not a black video box; video answers still play as video. A role with no public job title shows no subtitle under the name — never the internal title. `convex/shares.test.ts` |
| ID3c | **Highlights on a share link** (PR #46) | Open the share link of a report that has highlights | **Worth watching** lists the same moments as the recruiter page, under the player, and each starts playback at its second. The AI disclaimer is still there. `convex/shares.test.ts` |
...
| ID5 | Revoke | Revoke… → the confirmation names the link's creation date and view count → Revoke the link; reload the shared page. Also try Cancel in the confirmation | "This link was revoked". Playback URLs stop being issued |
...
| ID6d | Share limits | Call `shares:create` with `expiresInDays` `0`, `366` or `1.5`; call `shares:sharedMediaUrls` 15 times in a minute on a live link | The first three are refused `invalid_expiry` (the dialog's 7 / 30 / never still work). Media: the 11th call onward answers `rate_limited` (`shareMedia`), keyed on the resolved share. `convex/hardening.test.ts`, `convex/shares.test.ts` |
| ID7 | Search | ⌘K / Ctrl+K, or the search button in the header (an icon on a phone), then type three letters of a candidate's name | A modal list opens on every screen size; arrow keys move through results and Enter opens one; Escape closes it and the next opening starts empty. Finds candidates across roles. A member who is not on a role's team does **not** see its candidates here (M8) |
| ID8 | **Removal ends access** | Put member B on a role's team, have B create another role and a report share link, remove B, complete an interview on each, re-invite B as a plain member | B receives no "report ready" email while removed; B's share link now shows "This link was revoked" (h03); after re-invite B does **not** see the first role (the team row went with the membership), nor the role B created: no Team, Archive or Copy the link on it either (the creator's seat went too, T17-2). B's assistant conversations that read candidates of either role are gone |
...
| ID8c | **Removal keeps the credit** | Have admin B set a decision on a candidate and send an invitation, then remove B | The decision line, B's entries in the decision history and the pending invitation still name B, greyed and italic with "(removed)" / "(retiré)"; the invitation itself reads "No longer valid" (I12). Delete B's account: all three read "Former member" / "Ancien membre". No address appears that was not shown before |
...
| IE7 | Deleting a role takes its media | Record an intro and a question prompt on a role with no candidates, then record another take and close the tab before it saves; delete one question, then the role | Every object is gone from the bucket, not just the rows — the unsaved take too, named in `pendingMediaKeys` from the moment its upload URL was issued. `convex/media.test.ts` |
...
| IE8b | **An unsaved CV is erased too** | Choose a CV, cut the network once its upload has landed but before it is attached (or close the tab), then Delete everything | The `cv.*` object is gone from the bucket: its key was written to `sessions.pendingDocumentKeys` before the upload URL was signed. `convex/documentGate.test.ts` |
...
| IE9b | Assistant threads from before erasure tracking are gone | After deploying, Convex dashboard → Data → `migrations` | Within two hours of the deploy the `purgeLegacyAssistantThreads` row has `doneAt`; the agent component's `threads` table holds nothing created before its `cutoff`; logs show `[migrations] purgeLegacyAssistantThreads done` once, and later cron ticks log nothing |
