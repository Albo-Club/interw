...
| #  | Scenario | Steps | Expected |
| -- | -------- | ----- | -------- |
| IB1 | Invitation | Role → Candidates → Invite → one name + address | Email arrives; the link is `/s/{token}`. On a role with no public job title, the subject and body say "your interview" and never the internal title — same for the "interview sent" email (`convex/interview.test.ts`) |
