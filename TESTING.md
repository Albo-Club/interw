...
| S9 | Vercel build log (Production, each project)        | Shows `convex deploy` running before the Vite build, with `VITE_CONVEX_URL` and `VITE_CONVEX_SITE_URL` set by the CLI. A Preview build shows **no** `convex deploy` |
