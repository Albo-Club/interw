...
| IB1b | **Public candidate link** | Active role → **Create candidate link** → **Copy candidate link**; open it in a private window, enter a name + address, **Start** | Lands on the `/s/{token}` welcome screen greeting that name; no email is sent. The candidate shows up in the role's table like an invited one and goes on through IB4 → IB16 unchanged. Submitting the same address again opens a **second** session, never the first one back. Button hidden on a draft, archived or past-deadline role. `convex/apply.test.ts` |
