...
| B12 | Candidate bundle budget | `pnpm build:app && pnpm bundle:budget` | Prints the gzip size of what `/s/$token/interview` loads and its largest chunks; exit 0 at or under 270 KiB (265.3 KiB after the changelog copy left the i18n bundle) |
...
| I12 | Admin B invites `carol@test.local`, then B is removed or made a plain member | Carol's link shows the "not found" card and the invitation is no longer offered in her onboarding. Another admin's **Resend** makes them the inviter and the link works again. `convex/invitations.test.ts` |
...
| ID8c | **Removal keeps the credit** | Have admin B set a decision on a candidate and send an invitation, then remove B | The decision line, B's entries in the decision history and the pending invitation still name B (the invitation's link no longer works — I12), greyed and italic with "(removed)" / "(retiré)". Delete B's account: all three read "Former member" / "Ancien membre". No address appears that was not shown before |
