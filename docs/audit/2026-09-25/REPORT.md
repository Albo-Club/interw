...
|---|---|---|---|---|
| V1 | The e2e job deploys any same-repo PR branch with `CONVEX_DEPLOY_KEY`, without checking which deployment the key names | `ci.yml:3-6`, `:58-87`; `package.json:15` | **Settled by the owner (2026-09-25):** the key is a `prod:` key of the **`interw-staging`** Convex project, not of production. PR branches therefore deploy to staging, as designed. The same key sits in the Claude Code environment, which is why the T01–T16 sessions saw a `prod:` prefix and pushed nothing | Remaining risk: an unreviewed PR branch can overwrite staging's functions and schema. The deployment-name assertion in §5 still makes a later swap to the real production key fail loudly |
