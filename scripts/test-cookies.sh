...
#   BASE_URL=https://interw.team … pnpm test:cookies     # prod (expect Secure)
...
SESSION_LINE=$(echo "$COOKIE_LINES" | grep -i "interw\." | head -1 || true)
if [[ -z "$SESSION_LINE" ]]; then
  echo "FAIL: no interw.* cookie set on sign-in"
  echo "$COOKIE_LINES"
  exit 1
fi
